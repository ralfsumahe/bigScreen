...
//					option.title.text = data.title;
	 ...
	 * @returns
	 */
	function createZxt(domId,zxt) {
		var paramsList = ["linetype","isareastyle","isqx","max","min","title","xtitle","ytitle","datatitles"];
		...
		var colors = ['#45CCDE','#F8C72D','#58B04D','#E52E53','#79EB1F'];
...
//				text:'随时间变化的折线图',
	...
	function drawZxt(option,data,params,zxtChart){
		...
		var isAreaStyle = params["isareastyle"]!=false?true:params["isareastyle"];
...
//		option.title.text = data.title;
		...
		for(var i = 0;i < data.dataTitles.length;i++){
			var st = data.datas[i];
			var lineType1 = lineType;
			var color1 = colors[i];
			var isAreaStyle1 = isAreaStyle
